// Tool: Bash
// Original name variable in snippet: YV
// Original prompt function in snippet: y70
// Original schema variable in snippet: TF2 or D15 (Ni() ? D15 : TF2)

// Simulated values from functions in the snippet
const MAX_TIMEOUT_MS = 600000; // From FI1()
const DEFAULT_TIMEOUT_MS = 120000; // From Xb()
const MAX_OUTPUT_LENGTH = 30000; // From cn()
const RG_PATH = "/usr/bin/rg"; // Placeholder for Jx()
const CO_AUTHORED_COMMIT_MSG = `\uD83E\uDD16 Generated with [Claude Code](https://docs.anthropic.com/en/docs/claude-code)

   Co-Authored-By: Claude <noreply@anthropic.com>`; // From iR4().commit
const CO_AUTHORED_PR_MSG = `\uD83E\uDD16 Generated with [Claude Code](https://docs.anthropic.com/en/docs/claude-code)`; // From iR4().pr
...
const toolDefinition = {
  ...
  // Zod schema from snippet (assuming Ni() is true):
  // D15 = TF2.extend({
  //   sandbox: i.boolean().optional().describe("whether to run this command in sandboxed mode..."),
  //   shellExecutable: i.string().optional().describe("Optional shell path to use instead of the default shell...")
  // })
  // TF2 = i.strictObject({
  //   command: i.string().describe("The command to execute"),
  //   timeout: i.number().optional().describe(\`Optional timeout in milliseconds (max \${FI1()})\`),
  //   description: i.string().optional().describe(\` Clear, concise description of what this command does in 5-10 words...\`)
  // })
  inputSchema: {
    type: "object",
    properties: {
...
Output: Creates directory 'foo'\`,
  ...
  // Other properties from the snippet's tool structure (N4):
  // isReadOnly(A){...},
  // userFacingName(A){...},
  // isEnabled(){return!0},
